...
export function getMetadata(prototype: any): FieldMetadata[] {
  ...
  return properties.filter(isMetadataPresentForField(prototype)).map(v => getMetadataFromField(prototype, v));
}

function isMetadataPresentForField(target: Object): (propertyKey: string) => boolean {
  return propertyKey =>
...
function getMetadataFromField(target: Object, modelPropertyKey: string): FieldMetadata {
  const jsonPropertyName = Reflect.getMetadata(__FIELD_JSON_NAME_METADATA_KEY, target, modelPropertyKey);
  const serializer = Reflect.getMetadata(__FIELD_SERIALIZER_METADATA_KEY, target, modelPropertyKey);
  return {
